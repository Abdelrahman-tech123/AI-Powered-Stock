...
import { User, Mail, Lock, ArrowLeft, TrendingUp, ShieldCheck, HelpCircle } from "lucide-react";
...
export default function RegisterPage() {
    ...
    return (
        <div className="min-h-screen bg-[#fafafa] text-slate-900 flex flex-col lg:flex-row relative overflow-hidden font-sans selection:bg-slate-900 selection:text-white" dir="rtl">
            ...
            <div className="flex-1 flex flex-col justify-between p-8 lg:p-16 relative z-10 lg:max-w-[55%]">
                ...
                <div className="my-auto max-w-xl space-y-6 pt-16 lg:pt-0">
                    ...
                    <h1 className="text-4xl lg:text-5xl font-black tracking-tight text-slate-900 leading-[1.2]">
                        ابدأ رحلتك في رصد وتحليل الأسواق الاستثمارية
                    </h1>
                    <p className="text-slate-500 text-sm leading-relaxed font-light max-w-lg">
                        قم بإنشاء حسابك المخصص الآن للوصول إلى أدوات ذكية قادرة على قراءة تغيرات البورصة وتوفير تحليلات متقدمة تدعم قراراتك المالية اليومية.
                    </p>
                </div>
            ...
            {/* Left Panel: The Elegant Full-Height Registration Sheet (Takes 45% Width on Desktop) */}
                ...
                <div className="max-w-md w-full mx-auto px-8 py-16 lg:py-24 flex flex-col justify-center">
                    ...
                    <div className="mb-8">
                        ...
                        <p className="text-slate-400 text-xs font-light">
                            الخطوة الأولى للوصول إلى أدوات التحليل المتقدمة الخاصة بك.
                        </p>
                    </div>
                    ...
                    <form onSubmit={handleSubmit} className="space-y-4">
                        <div>
                            <label className="block text-slate-700 text-xs font-semibold mb-2 pr-0.5">الاسم الكامل</label>
                            <div className="relative group">
                                <User className="absolute top-1/2 -translate-y-1/2 right-4 text-slate-400 group-focus-within:text-slate-900 transition-colors duration-200" size={15} />
                                <input
                                    type="text"
                                    value={name}
                        ...
                        <div>
                            <label className="block text-slate-700 text-xs font-semibold mb-2 pr-0.5">كلمة المرور</label>
                            <div className="relative group">
                        ...
                        <button
                            type="submit"
                            disabled={loading}
                            className="w-full bg-slate-950 hover:bg-slate-900 text-white font-medium py-4 rounded-xl text-xs transition-all duration-300 disabled:opacity-50 flex items-center justify-center gap-2 mt-6 shadow-sm group/btn active:scale-[0.99]"
                        >
                            <span>{loading ? "جاري تهيئة حسابك..." : "انشاء حساب"}</span>
                            <ArrowLeft size={14} className="transition-transform duration-300 group-hover/btn:-translate-x-1" />
                        </button>
                    </form>

                    {/* Clean Simple Login Invitation Footer */}
                    <div className="text-center text-xs mt-10 border-t border-slate-100 pt-6">
                        <span className="text-slate-400">لديك حساب بالفعل؟</span>{" "}
                        <Link href="/login" className="text-slate-950 font-bold hover:underline ml-1">
                            سجل دخولك هنا
                        </Link>
                    </div>
                </div>
